...
class RunSetting:
    ...
    batch_size: int = 250
    weight_decay_coefficient: float = 5 * 10**-4  # 5 * 10**-5 ?
...
def get_run_setting(run_id: int):
    ...
    # Different models:
    if run_id == 10: rs.model_kind = "ffc"
    if run_id == 11: rs.model_kind = "conv"
    if run_id == 12: rs.model_kind = "std_conv"
